...
def load_inputs(m, switch_data, inputs_dir):
    ...
    # Group membership: resolve GROUP_NAMEs from gen_group_load_ratio.csv against
    # a named column of hierarchy.csv. The column name is read from
    # gen_zone_ratio_group_by.csv (one header row "hierarchy_col", one value row
    # with the column name, e.g. "hurdlereg"). This file is generated by
    # pg_to_switch from gen_zone_ratio_agg in scenario_management; users can edit
    # it to switch the grouping dimension without re-running pg_to_switch.
    # Pairs are filtered to zones in load_zones.csv so a project-wide hierarchy.csv
    # covering more zones than this model never causes a Pyomo domain error.
    group_ratio_path = apply_input_aliases(
        switch_data, os.path.join(inputs_dir, "gen_group_load_ratio.csv")
    )
    loaded_groups = False  # set True only when GEN_RATIO_GROUP_ZONES is populated
    if os.path.isfile(group_ratio_path):
        ...
        if not grl.empty:
            ...
            # gen_zone_ratio_group_by.csv specifies which hierarchy column to use
            group_by_path = apply_input_aliases(
                switch_data, os.path.join(inputs_dir, "gen_zone_ratio_group_by.csv")
            )
            if not os.path.isfile(group_by_path):
                print(
                    "WARNING gen_zone_ratio: gen_group_load_ratio.csv is present but "
                    "gen_zone_ratio_group_by.csv was not found; group constraints will "
                    "be skipped. Re-run pg_to_switch or create this file manually with "
                    "a single column 'hierarchy_col' and value matching a column in "
                    "hierarchy.csv (e.g. 'hurdlereg')."
                )
            else:
                ...
                # Walk up the directory tree to find hierarchy.csv if not in inputs_dir
                if not os.path.isfile(hier_path):
                    _search = os.path.abspath(inputs_dir)
                    for _ in range(6):
                        _parent = os.path.dirname(_search)
                        if _parent == _search:
                            break
                        _search = _parent
                        _candidate = os.path.join(_search, "hierarchy.csv")
                        if os.path.isfile(_candidate):
                            hier_path = _candidate
                            print(
                                f"gen_zone_ratio: using hierarchy.csv from {hier_path}"
                            )
                            break

                if not os.path.isfile(hier_path):
                    print(
                        "WARNING gen_zone_ratio: hierarchy.csv was not found; "
                        "group constraints will be skipped."
                    )
                else:
                    ...
                    if hierarchy_col not in hier.columns:
                        print(
                            f"WARNING gen_zone_ratio: column '{hierarchy_col}' from "
                            "gen_zone_ratio_group_by.csv not found in hierarchy.csv; "
                            "group constraints will be skipped."
                        )
                    else:
                        col_df = (
                        ...
                        # Filter to zones in this model; hierarchy.csv may cover a larger
                        # zone set than the current scenario uses.
                        zones_path = apply_input_aliases(
                            switch_data, os.path.join(inputs_dir, "load_zones.csv")
                        )
                        if os.path.isfile(zones_path):
                            model_zones = set(pd.read_csv(zones_path).iloc[:, 0].astype(str))
                            n_before = len(pairs)
                            pairs = [(g, z) for g, z in pairs if z in model_zones]
                            n_dropped = n_before - len(pairs)
                            if n_dropped:
                                print(
                                    f"gen_zone_ratio: dropped {n_dropped} zone(s) in "
                                    f"hierarchy.csv but not in this model's load_zones.csv"
                                )
    ...
    # Group-level constraints — only load when group membership was resolved;
    # loading params against an empty GEN_RATIO_GROUPS set causes a Pyomo error.
    if loaded_groups:
        _load_constraint_csv(
            switch_data,
            group_ratio_path,
