...
# Used to disaggregate the national wind growth limit to per-transreg caps.
BA_TO_TRANSREG = {
    "ISNE": "ISONE",
    "NYIS": "NYISO",
    "PJM":  "PJM",
...
# Onshore wind tech keys used in regional_resource_tags.yml
# (matches keys used in resource_tags.yml for MaxCapTag_WindGrowth)
ONSHORE_WIND_TECH_KEYS = {
    "LandbasedWind": 1,
    "onshore wind": 1,
...
# cap: RGGI (hard cap + CCR tiers, handled in carbon_policies_regional.py and
#      carbon_policies_ccr.csv generated by pg_to_switch.py)
# taxes: CA, WA (carbon price programs modelled as soft caps)
# NY excluded: NY power sector is expected to be outside the Cap & Invest programme
# Future:
# PA is supposed to enter RGGI eventually, but not clear when or with what target
# VA previously in RGGI and may rejoin — if so, update rggi_states.csv source and
# revise CCR pool sizes in switch.yml (carbon_ccr)
...
# %% #####################################
# Regional wind growth limits: disaggregate MaxCapTag_WindGrowth to per-transreg
# caps using a three-tier methodology:
#
#   ≤ 2028 — Pipeline method: baseline + (LBNL IA-executed MW + NON_IA_FRACTION
#             × non-IA active MW) from the interconnection queue. Grounds the
#             near-term cap in signed agreements and a discount on early-stage
#             projects, preventing the model from over-building in regions where
#             siting is severely limited (especially ISONE onshore wind).
#
#   2029–2030 — Regional methodology: baseline + historical share of national
#               14,490 MW/yr limit × years of growth. Uses EIA 860M 10-year
#               deployment shares by transreg. Slightly looser than the 2028
#               pipeline cap, reflecting pipeline expansion as projects advance.
#
#   ≥ 2031 — No regional constraint; only the national MaxCapTag_WindGrowth
#             applies. Regional supply-chain constraints are expected to ease
#             over a decade-plus horizon and historical deployment shares become
#             less informative.
#
# LBNL source: docs/analysis/RGGI/lbnl_queue_by_state.csv (generated by
#              docs/analysis/RGGI/lbnl_queue_vs_model.py from
#              LBNL_Ix_Queue_Data_File_thru2025.xlsx).

NON_IA_FRACTION = 0.25  # share of non-IA-executed queue expected to complete

# ── Compute per-transreg baselines and shares from EIA 860M ──────────────────
wind_860 = wb["operating"].query("energy_source_code_1 == 'WND'").copy()
wind_860["transreg"] = wind_860["balancing_authority_code"].map(BA_TO_TRANSREG)

national_annual_limit = 14490  # MW/yr — same value used for national cap above

# 10-year install totals by transreg (for regional methodology)
recent = wind_860[
    wind_860["operating_year"].between(last_hist_year - 9, last_hist_year)
].groupby("transreg")["winter_capacity_mw"].sum()

national_10yr = recent.sum()

# baseline (existing, not retiring)
baseline_regional = (
    wind_860[
        (wind_860["operating_year"] <= last_hist_year)
        & (wind_860["planned_retirement_year"].isna()
           | (wind_860["planned_retirement_year"] > last_hist_year))
    ]
    .groupby("transreg")["winter_capacity_mw"]
    .sum()
)

regional_annual_limit = (recent / national_10yr * national_annual_limit).fillna(0)

# ── Load LBNL queue data for 2028 pipeline caps ───────────────────────────────
lbnl_path = Path("docs/analysis/RGGI/lbnl_queue_by_state.csv")
if not lbnl_path.exists():
    raise FileNotFoundError(
        f"{lbnl_path} not found. Run docs/analysis/RGGI/lbnl_queue_vs_model.py first."
    )
lbnl_q = pd.read_csv(lbnl_path)
lbnl_wind28 = (
    lbnl_q[(lbnl_q["tech"] == "OnshoreWind") & (lbnl_q["year_cutoff"] == 2028)]
    .groupby("transreg")[["queue_mw", "ia_executed_mw"]]
    .sum()
)
...
# ── Build mapping: transreg → list of Switch zones ───────────────────────────
...
for transreg, annual_lim in regional_annual_limit.items():
    tag = f"MaxCapTag_WindGrowth_{transreg}"
    regional_tags.append(tag)
    baseline = float(baseline_regional.get(transreg, 0))
    desc = f"{transreg} Wind Growth Limit"

    for y in possible_model_years:
        if y <= 2028:
            # Pipeline method: IA-executed + NON_IA_FRACTION × non-IA active.
            # Only applies for transregs with LBNL queue coverage (ISONE/NYISO/PJM).
            # Other transregs have no LBNL data so the pipeline estimate would be
            # 0, collapsing to baseline and leaving zero headroom after the
            # predetermined floor in update_max_cap_files.py.  Skip the regional
            # cap for those regions; the national cap still applies.
            if transreg not in lbnl_wind28.index:
                continue
            pipeline_new = float(lbnl_wind28.loc[transreg, "pipeline_new_mw"])
            max_cap = baseline + pipeline_new
        elif y <= 2030:
            # Regional methodology: historical deployment share
            max_cap = baseline + annual_lim * (y - last_hist_year)
        else:
...
rrt = read_yaml(yaml_files["regional_tag_values"])
delete_yaml_keys(rrt, ["regional_tag_values", "*", "MaxCapTag_WindGrowth_*"])

for transreg, zones in transreg_zones.items():
    tag = f"MaxCapTag_WindGrowth_{transreg}"
    if tag not in regional_tags:
        continue   # no deployment data for this transreg, skip
    for zone in sorted(zones):
        add_yaml_key(
            rrt, ["regional_tag_values", zone, tag], ONSHORE_WIND_TECH_KEYS
        )

write_yaml(rrt, yaml_files["regional_tag_values"])

# ── Register regional tag names in model_definition.yml / resource_tags.yml ──
update_model_tag_names(regional_tags, "MaxCapTag_WindGrowth_")
